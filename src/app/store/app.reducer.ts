...
const reducer = createReducer(
    ...
    on(appActions.setLanguage, (state, action) => {
        const translateBirds = (card: BirdCard) => {
            ...
            if (!translated)
                return { ...englishBird, 'English name': englishBird['Common name'] }
            ...
            return { ...englishBird, 'English name': englishBird['Common name'], ...mergeContent, ...bonuses }
        }
        ...
        const translateBonuses = (card: BonusCard) => {
            ...
            if (!translated)
                return { ...englishBonus, 'English name': englishBonus['Bonus card'] }

            const mergeContent = translatedKeys.reduce((acc, key) =>
                (translated[key] && String(translated[key]).trim() ? { ...acc, [key]: String(translated[key]).trim() } : acc), {})

            return { ...englishBonus, 'English name': englishBonus['Bonus card'], ...mergeContent }
        }
